...
void seizeTimerAS() {
    ...
    if ( !timerInitialized ) {
        ...
        noInterrupts();
        takeOverTCA0(); 
        // TCA0.SINGLE.CTRLESET = TCA_SINGLE_CMD_RESET_gc;     // hard reset timer // Not rqeuired because of takeOverTCA0()
        TCA0.SINGLE.CTRLA = TCA_SINGLE_CLKSEL_DIV8_gc;      // 0,5µs per tic, timer disabled - not possible on megaavr because it influences millis()
        // TCA0.SINGLE.CTRLA = TCA_SINGLE_CLKSEL_DIV64_gc;      // 4µs per tic ( also used by millis() ), timer disabled
		//+-> megaavr Arduino default of prescaler TCA0 is 64, changes would influence millis()
		// this means 4µs per tic for TCA0
        // meagtinycore allows a prescaler of 2 for TCB1 for millis(), so changing TCA0 should not influence millis()
